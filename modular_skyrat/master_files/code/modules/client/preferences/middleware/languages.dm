...
/datum/preference_middleware/languages/get_ui_data(mob/user)
	...
	// This validates and eventually resets the language preferences if they're invalid.
	// It should not be here, in a proc tasked with fetching data for the UI.
	...
	for (var/language_path in GLOB.all_languages)
		var/datum/language/language = GLOB.language_datum_instances[language_path]
		if(language.secret)
			continue
...
/datum/preference_middleware/languages/proc/give_language(list/params, mob/user)
	...
	if(preferences.languages && preferences.languages.len == max_languages) // too many languages
		return TRUE
	preferences.languages[GLOB.language_path_by_name[language_name]] = LANGUAGE_SPOKEN
	return TRUE

/datum/preference_middleware/languages/proc/remove_language(list/params, mob/user)
	var/language_name = params["language_name"]
	preferences.languages -= GLOB.language_path_by_name[language_name]
	return TRUE
