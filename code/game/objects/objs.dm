...
/obj/examine_more(mob/user) //We handle each check individually, so we can have different descriptions for each.
	...
	if(examinepp_desc_mindshield)
		if(HAS_TRAIT(user, TRAIT_MINDSHIELD))
			. += "Your <span class='blue'>innate loyalty to the North Star</span> has given you insight here:<br> [examinepp_desc_mindshield]"
	if(examinepp_desc_antag)
		if(user.mind) //Sanity check
			...
			for(var/exppantag in examinepp_req_antags)
				...
				else if(examinepp_antag_allow_skillchip == TRUE && HAS_TRAIT(user, TRAIT_SEE_ALL_DESCRIPTIONS)) // WHY THE FUCK DO SKILLCHIPS APPLY THE TRAIT THREE TIMES WHAT THE FUCK
