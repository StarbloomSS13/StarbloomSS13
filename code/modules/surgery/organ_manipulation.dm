...
/datum/surgery_step/manipulate_organs/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	if(isorgan(tool))
		...
		display_pain(target, "You can feel your something being placed in your [parse_zone(target_zone)]!", target_zone = target_zone)
		...
		if(!length(organs))
			...
			if(user && target && user.Adjacent(target) && user.get_active_held_item() == tool)
				...
				display_pain(target, "You can feel your [target_organ] being removed from your [parse_zone(target_zone)]!", target_zone = target_zone)
			else
				return -1
...
/datum/surgery_step/manipulate_organs/success(mob/living/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results)
	...
	if(current_type == "insert")
		...
		display_pain(target, "Your [parse_zone(target_zone)] throbs with pain as your new [tool] comes to life!", target_zone = target_zone)
		...
		if(target_organ && target_organ.owner == target)
			...
			display_pain(target, "Your [parse_zone(target_zone)] throbs with pain, you can't feel your [target_organ] anymore!", target_zone = target_zone)
