...
/datum/surgery_step/add_prosthetic/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	...
	if(istype(tool, /obj/item/bodypart) && user.temporarilyRemoveItemFromInventory(tool))
		...
		give_surgery_pain(target, "You feel synthetic sensation wash from your [parse_zone(target_zone)], which you can feel again!", mechanical_surgery = TRUE, target_zone = target_zone)
		return
	else
		...
		give_surgery_pain(target, "You feel a strange sensation from your new [parse_zone(target_zone)].", mechanical_surgery = TRUE, target_zone = target_zone)
		qdel(tool)
