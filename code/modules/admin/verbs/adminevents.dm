...
/client/proc/admin_headset_message(mob/M in GLOB.mob_list, sender = null)
	...
	if (!sender)
		sender = input("Who is the message from?", "Sender") as null|anything in list(RADIO_CHANNEL_CENTCOM,RADIO_CHANNEL_EDICT)
		if(!sender)
			return
	...
	to_chat(H, span_hear("You hear something crackle in your ears for a moment before a voice speaks. \"Please stand by for a message from [sender == "Last Edict" ? "your benefactor" : "Central Command"]. Message as follows[sender == "Last Edict" ? ", agent." : ":"] <b>[input].</b> Message ends.\""), confidential = TRUE)
