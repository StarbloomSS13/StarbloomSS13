...
/client/verb/aooc(msg as text)
	...
	for(var/iterated_listener as anything in listeners)
		...
		to_chat(iterated_client, span_ooc("<font color='[color]'><b><span class='prefix'>AOOC:</span> <EM>[name]:</EM> <span class='message linkify'>[msg]</span></b></font>"))
...
/proc/toggle_aooc(toggle = null)
	...
	for(var/iterated_listener in listeners)
		var/client/iterated_client = iterated_listener
		to_chat(iterated_client, span_ooc("<B>The AOOC channel has been globally [GLOB.aooc_allowed ? "enabled" : "disabled"].</B>"))
